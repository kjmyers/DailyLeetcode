class Solution:
    def minHeightShelves(self, books: List[List[int]], shelfWidth: int) -> int:
        ...
        def dpHelper(i, remaining_shelf_width, max_height):
            ...
            if memo[i][remaining_shelf_width] != 0:
                ...
                option_1_height = max_height + dpHelper(i + 1, shelfWidth - current_book[0], current_book[1])
                if remaining_shelf_width >= current_book[0]:
                    # Calculate height of the bookcase if we put the current book on the current shelf
                    option_2_height = dpHelper(i + 1, remaining_shelf_width - current_book[0], max_height_updated)
